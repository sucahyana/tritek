...
const ProductDetail = () => {
    ...
    useEffect(() => {
        ...
        fetchProduct();
    }, [id, currentPage, perPage, fetchTrigger]);

    useEffect(() => { 
        if (fetchTrigger && dataProductRef.current) {
            dataProductRef.current.scrollIntoView({ behavior: "instant" });
        }
    }, [fetchTrigger, dataProductRef.current]);
    ...
    const content = () => {
        ...
        return (
            <div className="flex flex-col gap-8 justify-center w-full">
                ...
                />
                <div ref={dataProductRef}>
                    <DataProduct product={product} processes={processes} pagination={pagination} onPageChange={handlePageChange} onUpdate={handleDataProductUpdate} trigger={handleDataProductUpdate} materials={materials} />
                </div>
            </div>
        );
    };
